<?php
    class Category
    {
        ...
        {
            $tasks = array();
            $PDO_statement = $GLOBALS['DB']->query("SELECT task_id FROM categories_tasks WHERE category_id = {$this->getId()};");
            $returned_task_ids = $PDO_statement->fetchAll(PDO::FETCH_ASSOC);
            foreach($returned_task_ids as $id) {
                $task_id = $id['task_id'];
